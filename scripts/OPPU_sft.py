...
def contrastive_decoding(
    ...
    for _ in range(max_length):
        with torch.no_grad():
            ...
            processed_logits = logits_warper(all_input_ids, processed_logits)  
...
def create_dataset(user_history_data, tokenizer, prompt_template, args, system_prompt):
    data = []
...
def train(args):
    ...
    for user_idx in tqdm(range(len(test_data)), desc="Training"):
        ...
        dataset = create_dataset(user_history_data, tokenizer, prompt_template, args, SYSTEM_PROMPT)
        ...
        training_args = TrainingArguments(
            output_dir=f'./ckpt/OPPU_SFT_{args.max_epoch}/{args.task_name}/user_{user_idx}',
        ...
        save_path = f'./ckpt/OPPU_SFT_{args.max_epoch}/{args.task_name}/user_{user_idx}'
...
def inference(args):
    ...
    for run_idx in range(repeat_runs):
        ...
        file_suffix += f"max_epoch{args.max_epoch}" if args.max_epoch > 2 else ""
        file_suffix += "-sampling" if args.is_sampling else ""
        file_path = f"./output/{args.task_name}/OPPU-SFT-{model_name_short}-{file_suffix}-run_{run_idx+1}.json"
