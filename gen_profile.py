...
parser.add_argument('--task_name', type=str, default='news_headline', help='Task name for the model')
...
#             p[key] = get_first_k_tokens(str(p[key]), 200)
...
for user in tqdm(test_data):
    ...
    user_id = user['user_id'] if args.task_name in ("news_headline", "scholarly_title") else user['user_name']
    prompt_list_100.append(test_prompt)
    userid_list_100.append(user_id)
...
with torch.no_grad():
    for batch_idx, batch in tqdm(enumerate(batched_prompt_100), total=len(batched_prompt_100)):
        sentences = batch
        inputs = tokenizer(sentences, return_tensors="pt", padding=True, return_token_type_ids=False, truncation=True, max_length=4096)
        inputs = inputs.to(base_model.device)
